#Powershell version of maganders shell scrtip
...
#don't store credentials in scripts! this is just for demo purposes
$user="admin"
$passwd="secret"
#
...
echo > $file
#
if (( $pcinuse == "Y" ) -or ( $pcinuse == "y" )){
        Write-Host "Both Nutanix Prism Central $pcfqdn and Nutanix Prism Element $clusterfqdn will be used to collect information."
        Write-Host ""
        Write-Host "VM Name,VM Desription,VM Categories,VM Create Date,Total Number of vCPUs,Number of CPUs,Number of Cores per vCPU,Memory GB,vNUMA,Disk Usage GB, Disk Allocated GB,Number of VGs, VG Names,VG Disk Allocated GB,Flash Mode Enabled,AHV Snapshots,Local Protection Domain Snapshots,Remote Protection Domain Snapshots,IP Address/IP Addresses,Network Placement,AHV Host placement,Self Service Portal Project, Self Service Portal VM Owner, VM UUID " > $file
            }else{
        Write-Host "Nutanix Prism Element $clusterfqdn will be used to collect information."
        Write-Host ""
        Write-Host "VM Name,VM Description,Total Number of vCPUs,Number of CPUs,Number of Cores per vCPU,Memory GB,Disk Usage GB, Disk Allocated GB,Number of VGs, VG Names,VG Disk Allocated GB,Flash Mode Enabled,AHV Snapshots,Local Protection Domain Snapshots,Remote Protection Domain Snapshots,IP Address/IP Addresses,Network Placement,AHV Host placement, VM UUID " > $file
}
#
# Get VM uuids
#needs proper inspection cause we dont use python inside powershell
$vmuuids=`echo $getvms | python -m json.tool | grep -w uuid | awk -F":" '{print $2}' | awk -F"\"" '{print $2}'`
#
# Create the report
foreach ( $i in $vmuuids){ 
        # Get VM UUID
        $vmid=$i
        #
